...
export default function OverrideOverheadPage() {
  ...
  // ✅ 1. Update Perhitungan Asset ke Laporan Bulanan
  const handleUpdateFromAsset = async () => {
    ...
    const result = await Swal.fire({
      title: '🔄 Update Perhitungan Asset?',
      html: `
        <p>Anda akan <strong>menghitung ulang</strong> overhead dari Asset untuk semua laporan.</p>
        <p class="text-xs text-gray-400 mt-2">Overhead akan diambil dari total Asset.perMonth</p>
        <p class="text-xs text-gray-400">Override manual akan direset!</p>
      `,
      icon: 'question',
    ...
    if (result.isConfirmed) {
      ...
      try {
        const res = await fetch('/api/asset/update-laporan', {
          method: 'POST',
        });
        ...
        if (result.status === '✅ Berhasil!') {
          ...
          Swal.fire({
            icon: 'success',
            title: '✅ Berhasil!',
            text: `Berhasil update ${result.metadata?.updated || 0} laporan`,
            timer: 1500,
  ...
  // ✅ 2. Reset Semua Laporan Tahun Ini ke 0
  const handleResetLaporan = async () => {
    ...
    const laporanTahunIni = data.filter(d => {
      const year = parseInt(d.bulanStr.split('-')[0]);
      return year === tahunIni;
    });

    if (laporanTahunIni.length === 0) {
      Swal.fire({
        icon: 'info',
        title: 'Tidak ada laporan tahun ini!',
        text: `Belum ada laporan untuk tahun ${tahunIni}`,
      });
      return;
    }

    const result = await Swal.fire({
      title: `🔄 Reset Laporan ${tahunIni}?`,
      html: `
        <p>Anda akan <strong>mereset semua laporan</strong> tahun ${tahunIni} ke nilai awal.</p>
        <p class="text-xs text-gray-400 mt-2">Jumlah laporan: <strong>${laporanTahunIni.length}</strong> bulan</p>
        <p class="text-xs text-red-500">⚠️ Semua data yang diisi akan hilang!</p>
      `,
      icon: 'warning',
      showCancelButton: true,
    ...
    if (result.isConfirmed) {
      ...
      try {
        const res = await fetch('/api/laporan-bulanan/reset-tahun', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ tahun: tahunIni }),
        });
        ...
        if (result.status === '✅ Berhasil!') {
          ...
          Swal.fire({
            icon: 'success',
            title: '✅ Berhasil!',
            text: `Berhasil reset ${result.metadata?.reset || 0} laporan`,
            timer: 1500,
            showConfirmButton: false,
          });
        } else {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-wrap justify-between items-center gap-4 mb-6">
          ...
          <div className="flex flex-wrap gap-2">
            {/* ✅ Tombol Kembali ke Asset */}
            <Link
              href="/manage/overhead"
            ...
            {/* ✅ Tombol Refresh/Update dari Asset */}
            <button
            ...
            >
              ...
              🔄 Reset Laporan {new Date().getFullYear()}
            </button>
          </div>
        </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <ul className="text-xs text-blue-600 mt-1 space-y-1">
                <li>• <strong>Update dari Asset</strong> → Hitung ulang semua laporan dari total Asset.perMonth</li>
                <li>• <strong>Reset Laporan</strong> → Reset semua laporan tahun ini ke nilai 0</li>
                <li>• <strong>Edit per baris</strong> → Ubah nilai overhead per bulan secara manual</li>
                <li>• <strong>Set 0 per baris</strong> → Set 0 overhead bulan tertentu</li>
                <li>• <strong>Reset per baris</strong> → Kembalikan ke default (hanya jika sudah di-override)</li>
              </ul>
            </div>
          </div>
        </div>
